...
pub fn main(){
    ...
    /*벡터 내적
     ...
     공식을 보면 내적이 동일한 차원의 두 벡터 사이에만 성립함을 알수 있습니다.
          ...
          */
          ...
          let v: ndarray::ArrayBase<ndarray::OwnedRepr<i32>, ndarray::Dim<[usize; 1]>> = arr1(&[1,2,3,4]);
          let w = arr1(&[5,6,7,8]);
          //내적을 구하는 함수
          //np.dot()함수는 실제로 벡터-내적을 구현하지는 않습니다.여러 내적으로 이루어진 행렬곱셈을 구현했습니다.

          let final_mat: i32 = v.dot(&w);
   ...
   println!("{} {}",res1,res2);
    ...
    벡터크기는 엄격하게 양수(0벡터는 ||0||= 0으로 제외)지만 각의 코사인 값은 -1과 +1사이입니다.
    ...
    */
      ...
      */
      ...
      let a = arr1(&[5.0, 4.0, 8.0, 2.0]);
      let b = arr1(&[1.0, 0.0, 5.0, 0.0]); // 크기를 a 배열과 동일하게 수정
    ...
    외적은 스칼라 대신 행렬을 생성합니다
    외적은 두벡터는 차원이 달라도 되지만 내적의 두벡터는 차원이 같아야 합니다.
    외적은 vwᵀ로 나타냅니다(벡터가 열방향라고 가정하며  외적은 열에 행을 곱하는 것입니다.)
    ...
    즉 투영거리가 최소가 되도록 벡터b를 벡터a에 투영합니다.그점은 a의 크기를 줄인 즉 ℬa가됩니다.
    그러면 이제 스칼라 ℬ를 찾으면 댑니다

    중요한 것은 벡터뺼셈으로 b에서 ℬa까지의 선을 정의할수 있다는 점입니다.이선에 문자(벡터c)를 부여할수도 잇지만 먼저 답을 차기위해 뺄셈으로 표현하겟습니다.
    핵심은 b의 머리와 a가 직각으로 만나도록 그리면 b와 가장 가까운 a위의 점을 찾을수 있다는 점입니다.
    원점 b의 머리 ,ℬa로 이루어진 삼각형을 직관적으로 상상해보면 b에서 ℬa까지의 선의 길이는 각  ∠ℬa 가 90º보다 작거나 90º보다 클수록 길어집니다.
    종합해보면 b - ℬa가 ℬa와 직교한다는 것을 추론해볼수 있습니다.
    즉 이 벡터 들은 수직이며 둘사이의 내적이 0이 되어야 합니다.

    이제ℬ를 구하기 위해 약간의 대수학을 적용해보면 


    간단한 기하학적 그림과 공식 약간의 대수학을 적용하여 최소거리로 선에 투영하는 공식을 발견했습니다.
    이를 직교투영법이라고 하며 선형모델을 푸는데 잘알려진 최소제곱식,통계학,머신러닝등의 많은 응용분야에서 기초가됩니다.


    최소거리 투영은 벡터 분해를 배우기 위해꼭 필요한 기초 내용입니다.
    목표벡터와 기준벡터라는 두개의 벡터를  설정하면  목적은 목표벡터를 두개의 다른벡터로 분해하는것입니다.
    그 두개의 벡터의 합은 목표 벡터가 되고 하나의 벡터는 기준벡터와 직교하지만 다른벡터는 기준벡터와 평행합니다.

    수학계산을 시작하기 전에 용어를 잡으면
    목표벡터를 t,기준벡터를 r라고 부르며 목표벡터로부터 만들어진 두 벡터는 수직성분 tᅩᵣ 과 평생성분 t||ᵣ 로 각각 표시합니다.

    평행성분을 정의하면 r과 평행한 벡터는 r의 크기를 조정한 벡터는 분명히 r과 평행합니다.
    직교 투영공식을 적용하기만 하면 't || ᵣ를찾을수 잇습니다.


    여기서는 크기를 조절한 ℬr 을 계산합니다.
