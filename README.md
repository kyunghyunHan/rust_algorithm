...
- 높이가 h인포화이진트리늬 노드개수는 2^h-1이고 이준단말노드의개수는 2^h^-1이고 비단말노드의개수는 2^h^-1-1이다
...
- 연산
- 성능= O(nm)
- x와 y사이의 편집거리는 이들의 부분 문자열 사이의 편집거리를 포함
- p(i,j):E(i,j)로 선택되는 최솟값이 어떤 연산으로 결정되는지를 표시
...
- 내부정렬<->외부정렬
- 내부정렬 알고리즘의 구분(비교기간알고리즘<=>데이터 분포 기반 알고리즘)
- 안정적알고리즘
- 제자리정렬
...
- 종류:외판원문제,0/1 배낭문제,CNF만족성문제,해밀토니언 사이클문제,궤체우기문제,파티션무제,클리크판정문제,버텍스 커버문제
...
- O(n^2)
- 최소법,최선법,감소순최소법,최소순 최선법-

## 근사알고리즘)버텍스 커버문제

- 임의의 간선을선택하여 이와 맞닿은 두정점을 포함시키고 두정점에 부수된 간선을 제거하는 과정을 반복
- O(|E|)
